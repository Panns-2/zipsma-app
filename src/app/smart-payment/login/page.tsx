...
export default function SmartPaymentLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!schoolId.trim() || !staffId.trim()) {
      toast({ title: 'Error', description: 'Please enter both School ID and Staff ID.', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      // Allow Cashier, Administrator, and Accountant to use Smart Payment
      const allowedRoles = ['Cashier', 'Administrator', 'Accountant'];
      if (!allowedRoles.includes(role)) {
        throw new Error('You do not have permission to access Smart Payment.');
      }

      // Save session locally to persist forever
      localStorage.setItem('smart_payment_session', JSON.stringify({
        schoolId: upperSchoolId,
    ...
    <main className="flex min-h-screen w-full flex-col items-center justify-center bg-slate-50 p-4">
      <div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Smart Payment Portal</h1>
          <p className="text-gray-500 text-sm text-center mt-2">
            Securely log in to process student payments.
          </p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <Button type="submit" className="w-full h-12 text-lg font-semibold mt-4 bg-emerald-600 hover:bg-emerald-700 text-white" disabled={isLoading}>
            {isLoading ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <><Banknote className="mr-2 h-5 w-5" /> Launch Terminal</>}
          </Button>
        </form>
      </div>
    </main>
