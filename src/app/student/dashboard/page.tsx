...
import {
  ...
  isDailyTransaction,
  LedgerTransaction,
  calculateStudentTotalBalance,
  LibraryResource,
  getLibraryResourcesForClass,
  StudentReport,
...
import {
  ...
  QuizQuestion,
  ExplainConceptInput,
} from "@/ai/flows/student-assistant-flow";
        ...
        <Card className="border-red-200 bg-red-50 text-red-900 shadow-sm">
          <CardHeader className="py-4">
            ...
            <CardDescription className="text-red-700 text-xs">
              Your browser is blocking notifications. Please enable them in your
              device settings (or "Add to Home Screen" if on iOS) to receive
              alerts.
            </CardDescription>
          </CardHeader>
        </Card>
      ...
      <Card className="border-none shadow-xl bg-gradient-to-r from-blue-600 to-indigo-700 text-white overflow-hidden relative">
        ...
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-blue-100 text-sm font-medium">
            Enable push notifications to receive real-time alerts for school
            announcements and fee reminders directly on your phone.
          </CardDescription>
        </CardHeader>
...
const ChildSwitcher = ({
  ...
  return (
    <div
      className={cn(
        "flex flex-wrap items-center gap-3 mb-8 p-3 bg-white/60 backdrop-blur-md rounded-xl border border-white/40 shadow-sm w-full md:w-auto md:inline-flex",
        className,
      )}
    >
        ...
        <SelectContent className="rounded-lg border-primary/20 shadow-xl p-1 bg-white/95 backdrop-blur-xl">
          {showOverview && (
            <>
              <SelectItem
                value="overview"
                className="font-bold py-3 cursor-pointer rounded-lg focus:bg-primary/5 focus:text-primary transition-colors"
              >
          ...
          {children.map((child) => (
            <SelectItem
              ...
              className="font-bold py-2.5 cursor-pointer rounded-lg focus:bg-primary/5 focus:text-primary transition-colors"
            >
              <div className="flex items-center gap-3">
...
function DashboardContent() {
  ...
  const [studentData, setStudentData] = useState<Student | null>(null);
    ...
    const handlePinChange = async () => {
        ...
        setPinError('');
        try {
            await updatePin(db, schoolId, urlId, true, newPin);
  ...
  const [homeworkHelperInput, setHomeworkHelperInput] = useState({
    question: "",
  });
    ...
    try {
      ...
      if (student) {
        ...
        if (student.parentPhone) {
          const siblings = await getStudentsByParentPhone(
            db,
            schoolId,
            student.parentPhone,
          );
  ...
  const fetchStudentData = async (
    studentId: string,
    preFetchedStudent?: Student,
  ) => {
    setIsRefreshing(true);
    try {
      const student =
        preFetchedStudent || (await getStudentById(db!, schoolId!, studentId));
      if (student) {
        const [
          announcementsData,
          events,
  ...
  const financialData = useMemo(() => {
    ...
    if (
      !allFeeCategories.some(
        (c) => c.id === "feeding" || c.name === "Feeding Fee",
      )
    ) {
      allFeeCategories.push({
        id: "feeding",
        name: "Feeding Fee",
        schoolId: schoolId || "",
        isDaily: true,
    ...
    const currentPeriodIndex = sortedPeriodsForIndex.findIndex(
      (p) => p.id === selectedPeriodId,
    );

    // Split ledger into Daily and Main
    const dailyLedger = fullLedger.filter((t) =>
      isDailyTransaction(t, allFeeCategories),
    );
    const mainLedger = fullLedger.filter(
      (t) => !isDailyTransaction(t, allFeeCategories),
    );
    ...
    const getPeriodBalances = (ledger: LedgerTransaction[]) => {
      ...
      const prevTransactions = ledger.filter((t) => {
        ...
        const tPeriodIndex = sortedPeriodsForIndex.findIndex(
          (p) => p.id === t.periodId,
        );
        return (
          tPeriodIndex < currentPeriodIndex && t.periodId !== selectedPeriodId
        );
      });

      const bf = prevTransactions.reduce(
        (sum, t) => sum + (Number(t.debit) || 0) - (Number(t.credit) || 0),
        0,
      );

      // Match Admin's Current Term logic (line 1532 in admin page.tsx)
      const currentTransactions = ledger.filter(
        (t) => !selectedPeriodId || t.periodId === selectedPeriodId,
      );

      const billed = currentTransactions.reduce(
        (sum, t) => sum + (Number(t.debit) || 0),
        0,
      );
      const paid = currentTransactions.reduce(
    ...
    const breakdown = [
      ...dailyData.currentTransactions,
      ...mainData.currentTransactions,
    ]
      .filter((t) => t.debit > 0)
    ...
    const dynamicFeedingCat = allFeeCategories.find(
      (c) =>
        c.isDaily &&
    ...
    let dailyAccrued = 0;
    const daysPresent = (studentData.attendance || []).filter(
      (a) =>
        a.attended && (!selectedPeriodId || a.periodId === selectedPeriodId),
    ).length;
    ...
    feeCategories
      ...
      .forEach((cat) => {
        ...
        const studentRate =
          (studentData.dailyFees || []).find((f) => f.categoryId === normId)
            ?.rate || 0;
  ...
  const familyArrearsData = useMemo(() => {
    ...
    const childrenWithArrears = familyChildren.map((child) => {
      const balanceInfo = calculateStudentTotalBalance(
        child,
        academicPeriods,
        selectedPeriodId,
        feeCategories,
      ...
      const dailyArrears = Math.max(
        0,
        balanceInfo.dailyData.balance + balanceInfo.dailyAccruedInfo,
      );
      ...
      return {
        ...
        mainArrears,
        dailyArrears,
        attendanceToday: (child.attendance || []).find(
          (a) => a.date === new Date().toISOString().split("T")[0],
        )?.attended
          ? "Present"
          : (child.attendance || []).find(
                (a) => a.date === new Date().toISOString().split("T")[0],
              )
            ? "Absent"
            : "Not Recorded",
  ...
  const handleGenerateExplanation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Could not get explanation.",
        variant: "destructive",
      });
    } finally {
          ...
          <Card className="w-full max-w-md">
            ...
            <CardContent>
              <p className="text-muted-foreground">
                The School ID or Student/Parent ID does not match any of our
                records.
              </p>
              <Button
  ...
  const generateReportCard = (report: StudentReport) => {
    ...
    doc.setFontSize(20);
    doc.text(schoolDetails?.name || "School Report Card", 105, 20, {
      align: "center",
    });
    ...
    autoTable(doc, {
      startY: 60,
      head: [
        ["Subject", "Class Score", "Exam Score", "Total", "Grade", "Remark"],
      ],
      body: tableData,
      theme: "grid",
    ...
    doc.text(
      `Teacher's Remark: ${report.remarks?.teacherRemark || "-"}`,
      14,
      finalY + 15,
    );
    doc.text(
      `Head's Remark: ${report.remarks?.headTeacherRemark || "-"}`,
      14,
      finalY + 25,
    );

    doc.save(
      `${studentData?.name.replace(/\s+/g, "_")}_Report_${report.term}.pdf`,
    );
  };
          ...
          <TabsContent value="academics-hub" className="mt-6 space-y-8">
             <div className="space-y-8">
              ...
              <CardHeader>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <CardDescription>
                      Assignments for {studentData.className}
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
                ...
                {homework.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {homework.map((hw, index) => (
                      <div
                        key={hw.id}
                        className={`relative p-4 rounded-lg shadow-md text-gray-800 transform rotate-[-2deg] hover:rotate-0 hover:scale-105 transition-transform ${homeworkColors[index % homeworkColors.length]}`}
                      >
                        <Pin className="absolute top-2 right-2 w-5 h-5 text-gray-600/70" />
                        <h3 className="font-bold text-lg mb-2">{hw.title}</h3>
                        <p className="text-sm mb-3 h-16 overflow-hidden">
                          {hw.description}
                        </p>
                        <p className="text-xs font-semibold text-primary">
                          Due by:{" "}
                          {new Date(
                            hw.dueDate + "T00:00:00",
                  ...
                  <div className="text-center py-16 text-green-700 bg-green-50 rounded-lg">
                    ...
                    <p className="mt-1">
                      You have no homework right now. Great job!
                    </p>
                  </div>
                )}
              </CardContent>
              ...
              <CardHeader>
                ...
                <CardDescription>
                  Track continuous assessment and exam scores over time.
                </CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart
                          data={studentReports
                            ...
                            .map((r) => ({
                              term: r.term,
                              total: r.summary?.totalMarks || 0,
                              avg: r.summary?.averageScore || 0,
                            }))}
                        >
                            ...
                            <CardContent className="p-4 pt-2">
                              <div className="flex justify-between text-sm mb-4">
                                ...
                                <span>
                                  Position:{" "}
                                  <span className="font-bold">
                                    {report.summary?.classPosition || "N/A"}
                                  </span>
                                </span>
                              </div>
                              <Button
              ...
              <CardHeader>
                ...
                <CardDescription>
                  Download textbooks, past papers, and reading materials.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {libraryResources.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {libraryResources.map((resource) => (
                      <div
                        key={resource.id}
                        className="flex items-center justify-between p-4 border rounded-xl shadow-sm bg-slate-50"
                      >
                        <div className="flex items-start gap-3">
                          <div className="mt-1">
                            {resource.type === "pdf" ? (
                              ...
                              <BookCopy className="text-blue-500 w-6 h-6" />
                            )}
                          </div>
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400 mt-1">
                              Added: {format(resource.dateAdded, "MMM d, yyyy")}
                            </p>
                          </div>
                        </div>
                        <Button
                  ...
                  <div className="text-center p-8 text-muted-foreground">
                    ...
                    <p>
                      No resources available for {studentData?.className} right
                      now.
                    </p>
                  </div>
                )}
          ...
          <TabsContent value="school-life-hub" className="mt-6 space-y-8">
              ...
              <CardHeader>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <CardDescription>
                      Upcoming term events and holidays
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
                  ...
                  {calendarEvents.length > 0 ? (
                    <div className="space-y-4">
                      {calendarEvents.map((event) => (
                        <div
                          key={event.id}
                          className="flex items-start gap-4 p-3 rounded-md bg-slate-50 border border-slate-100"
                        >
                    ...
                    <div className="text-center py-12 text-muted-foreground">
                      ...
                      <p className="mt-4">
                        The school calendar has not been updated yet.
                      </p>
                    </div>
                  )}
                </div>
            ...
            <section>
              <h3 className="text-xl font-bold text-primary flex items-center gap-2 mb-6 underline decoration-primary/20 decoration-4 underline-offset-8">
                <CalendarDays className="w-5 h-5 text-indigo-600" /> Attendance
                History
              </h3>
...
<div className="hidden md:flex flex-col items-center mt-12 gap-4">
          ...
          <div className="flex flex-wrap justify-center gap-4">
              ...
              <a
                href={
                  schoolDetails?.schoolEmail
                    ? `mailto:${schoolDetails.schoolEmail}`
                    : "#"
                }
              >
            ...
            <Button asChild>
              <a
                href={
                  schoolDetails?.schoolPhone
                    ? `https://wa.me/${schoolDetails.schoolPhone.replace(/\D/g, "")}`
                    : "#"
                }
                target="_blank"
      ...
      <ContactBar
        schoolPhone={schoolDetails?.schoolPhone}
            ...
            <>
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setRevisionInput({ topic: e.target.value })
                        }
                        required
                        disabled={isGenerating}
                      />
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setQuizGeneratorInput({ topic: e.target.value })
                        }
                        required
                        disabled={isGenerating}
                      />
                    ...
                    {generatedQuiz.length > 0 && (
                      <div className="mt-6 space-y-6">
                        <h4 className="font-semibold text-lg text-primary">
                          Generated Quiz on: {quizGeneratorInput.topic}
                        </h4>
                              ...
                              {q.options.map((option, oIndex) => {
                                ...
                                return (
                                  <Button
                                    ...
                                    className={cn(
                                      "w-full justify-start h-auto py-2 px-3 text-wrap",
                                      {
              ...
              <DialogFooter>
                ...
                <Button
                  ...
                  form={
                    activeTool.id === "homeworkHelper"
                      ? "homework-helper-form"
                      : activeTool.id === "revisionAssistant"
                        ? "revision-assistant-form"
                        : "quiz-generator-form"
                  }
                  disabled={isGenerating}
                >
