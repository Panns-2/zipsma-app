...
import { Student, FeeCategory, AcademicPeriod, postBulkParentMainPayment, calculateStudentTotalBalance } from '@/lib/data-store';
import { useToast } from '@/hooks/use-toast';
import { Auth } from 'firebase/auth';
import { Firestore } from 'firebase/firestore';
import { cn } from '@/lib/utils';

interface ParentBulkMainPaymentModalProps {
    isOpen: boolean;
    onClose: () => void;
    students: Student[];
...
export const ParentBulkMainPaymentModal: React.FC<ParentBulkMainPaymentModalProps> = ({
    isOpen,
    onClose,
    students,
    feeCategories,
    ...
    const childrenDebts = useMemo(() => {
        ...
        return selectedChildren.map(student => {
            const balanceInfo = calculateStudentTotalBalance(student, periods, period.id, feeCategories);
            return { student, debt: Math.max(0, balanceInfo.mainData.balance) };
        });
    }, [selectedChildren, period, periods, feeCategories]);
    ...
    const previewDistribution = useMemo(() => {
        ...
        const distribution = childrenDebts.map(c => ({ student: c.student, oldDebt: c.debt, payment: 0 }));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!selectedParentId || isNaN(amount) || amount <= 0) return;

        setIsSubmitting(true);
        try {
            await postBulkParentMainPayment(
                db, auth, schoolId, selectedParentId, amount, period, feeCategories, "Bulk Parent Main Payment"
            );
            toast({ title: "Payment Recorded", description: `Successfully recorded and distributed GH¢${amount.toFixed(2)}.` });
            onSuccess();
            onClose();
        } catch (error: any) {
            console.error("Bulk Parent Main Payment Error:", error);
            ...
            <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden border-none shadow-2xl rounded-3xl font-sans">
                <DialogHeader className="bg-indigo-600 text-white p-8">
                    <DialogTitle className="text-2xl font-black flex items-center gap-3">
                        ...
                        Parent Bulk Payment (Core Fees)
                    </DialogTitle>
                    <DialogDescription className="text-indigo-100 font-medium">
                        Record a lump sum payment from a parent. The system will automatically distribute it across all their children to clear termly/academic debts.
                    </DialogDescription>
                </DialogHeader>
                            ...
                            {previewDistribution.length > 0 && (
                                <div className="space-y-3">
                                    ...
                                    <p className="text-[10px] text-slate-500 font-medium italic">
                                        * Payments are applied to debts first. Any leftover amount is distributed equally as an advance credit.
                                    </p>
                                </div>
                            )}
