...
if 'updatePin' not in text:
    text = text.replace(
        "import { getStudentsByParentPhone, getStudentsByParentId,",
        "import { getStudentsByParentPhone, getStudentsByParentId, updatePin,"
    )
    if 'updatePin' not in text: # Fallback if import string differs
        text = text.replace(
            "import { getStudentsByParentPhone",
            "import { getStudentsByParentPhone, updatePin"
        )
...
states = """
    ...
    useEffect(() => {
        if (parentData && parentData.pinChangeRequired) {
    ...
    const handlePinChange = async () => {
        ...
        try {
            ...
            setParentData(prev => prev ? { ...prev, pinChangeRequired: false } : prev);
...
pin_modal = """
            ...
            <Dialog open={showPinModal} onOpenChange={(open) => {
                ...
                if (!parentData?.pinChangeRequired) setShowPinModal(open);
            }}>
